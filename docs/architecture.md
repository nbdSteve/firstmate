...
Because the watcher is detached, reaping that per-turn hook tree no longer stops it; Claude's `bin/fm-claude-sessionend-shutdown.sh` SessionEnd hook instead stops this home's watcher through the arm wrapper's `--shutdown` mode only on a genuine full-session teardown, skipping a `/clear` and an away-mode home.
