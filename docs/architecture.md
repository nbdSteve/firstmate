...
`bin/fm-watch-arm.sh` remains the verified arm wrapper for protocols that call it; it launches the watcher detached in its own session so a routine arm reap cannot drop supervision, still waits on it via the exec-preserved pid, verifies it is genuinely alive with a fresh liveness beacon, and prints an honest `started`, `attached`, or nonzero `FAILED` status.
...
Because the watcher is detached, reaping a per-turn hook tree or ending an arm cycle no longer stops it, and the watcher has no idle self-exit, so each runtime that exposes a genuine full-session-teardown signal stops this home's watcher through the arm wrapper's `--shutdown` mode on that signal, skipping an away-mode home and acting only when this session still owns the lock: Claude uses its `bin/fm-claude-sessionend-shutdown.sh` SessionEnd hook (also skipping a `/clear`), Pi uses its watcher extension's `session_shutdown` handler, and OpenCode uses its plugin's genuine-teardown path.
Grok arms via `.grok/hooks` Stop hooks and exposes no session-teardown hook surface, so a Grok primary's watcher cannot receive a deterministic teardown shutdown; it is a known limitation, and that watcher is stopped only by the next singleton self-eviction or a signal.
